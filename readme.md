...
## step 42 order delivery

1. create endpoint in backend, order_views.py
...
3. backto front, edit orderConstants.js
...
function OrderScreen() {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dispatch, navigate, orderId, successPay, userInfo, successDeliver]);
...
## step 43 product review

1. goto backend, update product_views.py, include review model
...
3. need to link there review with product, update serializer.py
...
## 1. What is `get_reviews`?

It’s a **`SerializerMethodField` callback function** — this is the _standard, official way_ to add custom/ computed fields in a DRF ModelSerializer.
...
When you name a field `reviews`, DRF will **automatically look for a function named `get_reviews`**

```python
def get_reviews(self, obj):
```

- `get_` + field name = **required naming convention**
- `obj` = the current `Product` object

---

## 2. Is this the standard way to get related reviews?
...
```python
def get_reviews(self, obj):
    reviews = obj.review_set.all()   # get all reviews for this product
    serializer = ReviewSerializer(reviews, many=True)
    return serializer.data
```

# There is ONLY ONE VALID FUNCTION NAME
...
## So you CANNOT use other function names — only get\_...

4. update models.py for the review model
...
6. backto frontend, update product_constants.js
...
## step 44 product search

1. create components SearchBox.jsx
...
2. insert searchBox inside header
...
4. update the productactions.js
...
export const listProducts =
  ...
  async (dispatch) => {
    ...
    try {
      dispatch({ type: PRODUCT_LIST_REQUEST });
      const { data } = await axios.get(`/api/products${keyword}`); // update the api with keyword
      dispatch({ type: PRODUCT_LIST_SUCCESS, payload: data });
    } catch (error) {
      dispatch({
        type: PRODUCT_LIST_FAIL,
...
5. changing the api, we need to goback to backend to update the api, update product_views.py
...
1. based on django's pagination, update product_views to include pagination
...
def getProducts(request):
    ...
    return Response({
        ...
        'page': products_paginated.number,
...
2. update frontend productreducers.js
...
4. want to add page and keywords in the searchbox
...
function SearchBox() {
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    if (keyword) {
      navigate(`/?keyword=${keyword}&page=1`); // make sure page is included
    } else {
      navigate(location.pathname);
    }
  };
...
7. update ProductListScreen.jsx
    ...
    <div>
      ...
      {loading ? (
        ...
        <div>
          ...
          <Paginate pages={pages} page={page} isAdmin={true} /> // add this line
        </div>
      )}
    </div>
  );
...
1. add endpoint in backend, edit product_views.py
...
3. back to frontend, update productConstants.js
...
7. create components, ProductCarousel.jsx
...
10. bug fixed for the productlistscreen create button
